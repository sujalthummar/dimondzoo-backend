...
exports.addCoupon = async (req, res) => {
  if (req.token.userid && mongoose.Types.ObjectId.isValid(req.token.userid)) {
    ...
    if (userData && userData.verified == true) {
      const { couponid, rule, promocode, description, start_time, end_time, discount } = req.body;
...
exports.validateCoupon = async (req, res) => {
  ...
  const couponData = await Coupon.findOne({promocode}).select("-createdAt -updatedAt -__v").lean();

  if (couponData && couponData != null) {
      if (currentTime >= couponData.start_time && currentTime <= couponData.end_time ) {
        return responseManager.onSuccess("Coupon data", {
